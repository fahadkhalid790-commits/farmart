    ...
    public function getProductCategoriesWithUrl(array $categoryIds = [], array $condition = [], ?int $limit = null): Collection
    {
        ...
        $query = ProductCategory::query()
            ...
            ->select([
                ...
                'parent_id',
                DB::raw("
                    ({$tablePrefix}slugs.prefix ||
                    CASE WHEN ({$tablePrefix}slugs.prefix IS NOT NULL AND {$tablePrefix}slugs.prefix != '') THEN '/' ELSE '' END ||
            ...
            ->when($this->isEnabledMultiLanguages(), function (Builder $query): void {
                $query
                    ...
                    ->addSelect(
                        DB::raw("
                            CASE WHEN st.key IS NOT NULL
                                THEN (st.prefix || CASE WHEN (st.prefix IS NOT NULL AND st.prefix != '') THEN '/' ELSE '' END || st.key)
                                ELSE (slugs.prefix || CASE WHEN (slugs.prefix IS NOT NULL AND slugs.prefix != '') THEN '/' ELSE '' END || slugs.key)
                            END as url
                        ")
                    );
            })
    ...
    public function applyQuery(Builder $query): Builder
    {
        if ($this->isEnabledMultiLanguages()) {
            return $query
                ...
                ->addSelect(DB::raw('CASE WHEN ct.name IS NOT NULL THEN ct.name ELSE ec_product_categories.name END as name'));
        }
